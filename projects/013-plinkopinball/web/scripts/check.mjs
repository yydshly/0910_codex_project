...
const web=fileURLToPath(new URL('../',import.meta.url)),out=resolve(web,'dist'),base=new URL('https://local.invalid/0910_codex_project/013-plinkopinball/'),pages=['index.html','understanding.html','sources.html'],ids=new Map();
...
for(const name of ['01-understanding.md','02-sources-and-verification.md'])if(await readFile(resolve(out,'notes',name),'utf8')!==await readFile(resolve(web,'../notes',name),'utf8'))throw new Error('Stale note '+name);
...
for(const file of ['app.mjs','physics.mjs','webmcp.mjs'])execFileSync(process.execPath,['--check',resolve(out,file)],{stdio:'inherit'});
...
console.log(`Plinkopinball check passed: three pages, ${count} local links/anchors, matching notes, scripts and simulation tests.`);
