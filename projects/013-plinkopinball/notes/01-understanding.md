# Plinkopinball：一个弹珠下落的 3D 游戏

Plinkopinball 是一个弹珠下落的 3D 游戏：点击投球，看小球穿过钉阵落到弹珠台，再用挡板和蓄力装置继续击球。

![作者原版游戏效果：木质钉板上的八颗弹珠、下方弹珠台和拱形轨道](../assets/gameplay-screenshot.png)

图 1：2026-09-10 在 [作者在线演示](https://codrops-demo-for-threejs-conference-ten.vercel.app/) 中静音进入并投下 8 球后截取。画面对应当日原站，不能据此确认与固定源码版本完全一致。

## 我们的理解结论

- **它是什么：** 一个具体的浏览器 3D 弹珠游戏演示，把 Plinko 的钉阵下落与 Pinball 的挡板击球组合在一起。
- **为什么画面精细：** 场景在 Blender 中制作并烘焙光照，再由 Three.js 显示；移动的小球使用单独的材质和阴影。
- **涉及哪些算法：** 重力与碰撞求解、固定时间步长、模型截面和轮廓提取、坐标转换、曲线运动与实例化绘制。画面是三维的，板面物理主要在两个二维世界中计算，轨道运动按一维路径处理。
- **适合怎么用：** 直接作为可玩的作品和活动页面，也适合学习 Three.js 场景、物理、输入与音效的协作方式。
- **可以扩展什么：** 给原游戏增加计分、关卡和主题；也可以借鉴其交互思路发展解谜、迷宫、打砖块、轨道、机关、分类、节奏与竞速。
- **我们实际做了什么：** 保留原版 3D 在线体验，另写独立二维原理实验和八种挑战玩法。三档难度、道具、连击、评级和本机成绩属于我们的扩展原型，不是上游已经提供的能力。
- **还缺什么：** 原库没有完整关卡、账号、排行榜或联机系统；精细三维的八种扩展游戏仍需进一步制作。下文按固定源码展开说明，扩展原型的具体规则与测试另见网页中的“八种玩法”。
