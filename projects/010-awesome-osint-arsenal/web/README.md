...
以工具查找为主的中文静态展示：753 条原始目录记录、中文分类和获取方式筛选、名称用途搜索、分页、六个场景、完整理解图和搜索更新边界。目录搜索位于页面前部，帮助按用途找到工具并了解获取方式。不会执行安装脚本或第三方工具。
...
未安装或实测第三方工具。场景仅介绍输入、预期输出与选择流程。网页搜索只查固定快照，没有互联网工具发现或上游自动同步能力。可选 WebMCP 没有真实浏览器上下文验证，不影响普通页面使用。
