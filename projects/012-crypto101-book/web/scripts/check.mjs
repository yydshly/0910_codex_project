...
const pages=(await readdir(out)).filter(f=>f.endsWith('.html'));assert.equal(pages.length,21);
...
console.log('Crypto 101 checks passed: coverage, 21 pages, '+references+' local references, source anchors, glossary, downloads, progress normalization and two teaching calculations.');
