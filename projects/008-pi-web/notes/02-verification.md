...
外部摘要改为“能力 → 同类产品 → 对当前工作的用途”，补充 Claude Code 和 Cursor Agent 官方概述；增加原创 entry-guide.png / entry-guide.svg，目视检查其文字与布局。根 README、线上导航和展示页使用同一引导素材，保留原完整技术图。用途属于研究判断，未声称实测收益。后续发布版本以 deployment.json 为准。
